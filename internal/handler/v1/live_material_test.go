...
type mockLiveMaterialService struct {
	createFn               func(ctx context.Context, createdBy uint, name, liveURL, remark, ext, urlType string) (*model.LiveMaterial, error)
...
func (m *mockLiveMaterialService) Create(ctx context.Context, createdBy uint, name, liveURL, remark, ext, urlType string) (*model.LiveMaterial, error) {
	if m.createFn != nil {
		return m.createFn(ctx, createdBy, name, liveURL, remark, ext, urlType)
	}
	return nil, nil
}
